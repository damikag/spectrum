...
/**
 * Make a URL string is a spectrum.gq URL
 */
export default (url: string): boolean => {
  ...
  try {
    ...
    // hostname might be spectrum.gq or subdomain.spectrum.gq, so we use .endsWith
    // We don't just check .contains because otherwise folks could make spectrum.gq.mydomain.com
    const IS_SPECTRUM_URL =
      hostname === 'spectrum.gq' || hostname === 'alpha.spectrum.gq';
    const IS_LOCALHOST = hostname === 'localhost';
    const IS_HTTP = protocol === 'https:' || protocol === 'http:';
    // Make sure the passed redirect URL is a spectrum.gq one or (in development) localhost
