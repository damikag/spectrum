      ...
      .then(windowClients => {
        for (let i = 0; i < windowClients.length; i++) {
          ...
          if (
            ...
            // eslint-disable-next-line
            !(self.registration.scope.indexOf('http://spectrum.gq') === 0)
          ) {
            return;
          }
        }
  ...
  event.waitUntil(
    ...
    self.clients
      ...
      .then(function(clientList) {
        // If there is an open spectrum.gq window navigate to the notification href
        if (clientList.length > 0) {
          return clientList[0]
            .focus()
            .then(client => client.navigate(urlToOpen));
        }
        // If there's no open spectrum.gq window open a new one
        // eslint-disable-next-line
